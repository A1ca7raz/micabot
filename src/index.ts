...
function trimLe(str: string): string {
  if (str.includes('了')) return str
    ...
    bot
      ...
      .filter(
        ...
        async (ctx: Context) => {
          ...
          switch (parts.length) {
            case 1:
              text = `${op1} ${trimLe(parts[0])} ${op2}！`
              break

            case 2:
              if (['把', '被', '对'].includes(parts[0]))
                text = `${op1} ${parts[0]} ${op2} ${trimLe(parts[1])}`
              else
								text = `${op1} ${trimLe(parts[0])} ${op2} 的${parts[1]}！`
              break

            case 3:
              if (parts[0] == '对')
                text = `${op1} 对 ${op2} ${trimLe(parts[1])}${parts[2]}！`
              else if (['把', '被'].includes(parts[0]))
                text = `${op1} ${parts[0]} ${op2} 的${parts[1]}${trimLe(parts[2])}！`
							else
								text = `${op1} ${trimLe(parts[0])} ${op2} 的${parts[1]}${trimLe(parts[2])}！`
              break
          ...
          if (operated.id === ctx.me.id)
            text = `${parseUser(operator!)}，呀打哟～`
